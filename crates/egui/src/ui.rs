...
impl Ui {
    ...
    pub fn new_child(&mut self, ui_builder: UiBuilder) -> Self {
        ...
        let (id, next_auto_id_salt) = if global_scope {
            (id_salt, id_salt.value())
        } else {
            (self.id, self.next_auto_id_salt)
        };

        let stable_id = id.with(id_salt);
        let unique_id = stable_id.with(next_auto_id_salt);
