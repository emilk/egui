...
/// Everything you need to paint egui with [`wgpu`] and a [`raw_window_handle`]
///
...
impl Painter {
    ...
    /// Updates (or clears) the raw window handle associated with the [`Painter`]
    ///
    ...
    ///
    /// `width_in_pixels` and `height_in_pixels` are the physical size of the window's client area.
    /// If the `window` is called with `None` then these values are ignored.
    ///
    /// # Portability
    ///
    /// _In particular it's important to note that on Android a it's only possible to create
    /// a window surface between `Resumed` and `Paused` lifecycle events, and Winit (and most
    /// likely any windowing library) will panic on attempts to query the raw window handle while paused._
    ///
    ...
    pub async unsafe fn set_window<W>(
        &mut self,
        window: Option<&W>,
        width_in_pixels: u32,
