...
///
/// For styling purposes, you should consider using [`FontStack`] to specify a set of multiple fonts.
...
pub enum FontSlant {
    ...
    Normal,
    Italic,
    // TODO(valadaptive): oblique?
}

impl FontSlant {
    /// Helper for converting an "is italic?" boolean to a [`FontSlant`].
    pub fn italic(is_italic: bool) -> Self {
        if is_italic {
            Self::Italic
        } else {
            Self::Normal
    ...
    pub(crate) fn as_parley(&self) -> parley::FontStyle {
        match self {
            Self::Normal => parley::FontStyle::Normal,
            Self::Italic => parley::FontStyle::Italic,
        }
    }
}
...
impl Hash for FontWeight {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        emath::OrderedFloat(self.0).hash(state);
    }
}
...
impl Hash for FontVariations {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        ...
        for setting in settings.iter() {
            ...
            emath::OrderedFloat(setting.value).hash(state);
        }
    }
}
...
pub mod named_variants {
    ...
    pub struct NamedFontVariants {
        ...
        pub stylesets: Cow<'static, [u16]>,
        /// Enables display of font-specific character variants.
        pub character_variants: Cow<'static, [u16]>,
    ...
    impl Default for NamedFontVariants {
        fn default() -> Self {
            Self {
    ...
    impl From<NamedFontVariants> for FontFeatures {
        fn from(value: NamedFontVariants) -> Self {
...
impl FontStyle {
    ...
    pub fn with_slant(&self, slant: FontSlant) -> Self {
        Self {
            slant,
            ..self.clone()
        }
    ...
    pub fn with_named_variants(&self, named_variants: NamedFontVariants) -> Self {
        Self {
...
impl Hash for FontStyle {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        ...
        emath::OrderedFloat(self.size).hash(state);
...
impl TextFormat {
    pub(crate) fn as_parley(&self) -> parley::TextStyle<'static, Color32> {
        parley::TextStyle {
            font_stack: self.font.family.as_parley(),
            font_size: self.font.size,
            font_width: self.font.width.as_parley(),
            font_style: self.font.slant.as_parley(),
            font_weight: self.font.weight.as_parley(),
            font_variations: self.font.variations.as_ref().map_or_else(
                || parley::FontSettings::List(Cow::Borrowed(&[])),
                |s| s.as_parley(),
            ),
            font_features: self.font.features.as_ref().map_or_else(
                || parley::FontSettings::List(Cow::Borrowed(&[])),
                |s| s.as_parley(),
            ),
            locale: None,
            ...
            line_height: self.line_height() / self.font.size,
            word_spacing: 0.0,
...
impl TextFormat {
    #[inline]
    pub fn simple(font: FontStyle, color: Color32) -> Self {
        Self {
            font,
            color,
            ..Default::default()
    ...
    pub(crate) fn line_height(&self) -> f32 {
        self.line_height.unwrap_or(self.font.size)
    }
}
