...
void main() {
    #if SRGB_TEXTURES
        vec4 texture_in_linear = texture2D(u_sampler, v_tc);
    #else
        vec4 texture_in_linear = linear_from_gamma_rgba(texture2D(u_sampler, v_tc));
    #endif

    // The texture sampler is sRGB aware, and OpenGL already expects linear rgba output
    // so no need for any sRGB conversions here:
    gl_FragColor = v_rgba * texture_in_linear;

    gl_FragColor = gamma_from_linear_rgba(gl_FragColor);
}
