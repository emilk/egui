...
fn color_button(ui: &mut Ui<'_>, color: Color32, open: bool) -> Response {
    ...
    response.widget_info(ui.ctx, || WidgetInfo::new(WidgetType::ColorButton));
...
fn color_text_ui(ui: &mut Ui<'_>, color: impl Into<Color32>, alpha: Alpha) {
    ...
    ui.horizontal(|ui| {
        ...
        if ui
            .button("📋")
            .on_hover_text(ui.ctx, "Click to copy")
            .clicked()
        {
        ...
        if alpha == Alpha::Opaque {
            ui.label(format!("rgb({}, {}, {})", r, g, b))
                .on_hover_text(ui.ctx, "Red Green Blue");
        } else {
            ui.label(format!("rgba({}, {}, {}, {})", r, g, b, a))
                .on_hover_text(ui.ctx, "Red Green Blue with premultiplied Alpha");
        }
    });
}
...
fn color_picker_hsvag_2d(ui: &mut Ui<'_>, hsva: &mut HsvaGamma, alpha: Alpha) {
    ...
    show_color(ui, *hsva, current_color_size).on_hover_text(ui.ctx, "Selected color");
    ...
    .on_hover_text(ui.ctx, "Hue");

    if false {
        color_slider_1d(ui, s, |s| HsvaGamma { s, ..opaque }.into())
            .on_hover_text(ui.ctx, "Saturation");
    }

    if false {
        color_slider_1d(ui, v, |v| HsvaGamma { v, ..opaque }.into()).on_hover_text(ui.ctx, "Value");
    }
...
pub fn color_edit_button_hsva(ui: &mut Ui<'_>, hsva: &mut Hsva, alpha: Alpha) -> Response {
    let popup_id = ui.auto_id_with("popup");
    let open = ui.memory().is_popup_open(popup_id);
    let mut button_response = color_button(ui, (*hsva).into(), open);
    if ui.style().explanation_tooltips {
        button_response = button_response.on_hover_text(ui.ctx, "Click to edit color");
    }
...
pub fn color_edit_button_srgba(ui: &mut Ui<'_>, srgba: &mut Color32, alpha: Alpha) -> Response {
    let mut hsva = color_cache_get(ui.ctx, *srgba);
...
pub fn color_edit_button_srgb(ui: &mut Ui<'_>, srgb: &mut [u8; 3]) -> Response {
...
/// Shows a button with the given color.
/// If the user clicks the button, a full color picker is shown.
pub fn color_edit_button_rgba(ui: &mut Ui<'_>, rgba: &mut Rgba, alpha: Alpha) -> Response {
...
/// Shows a button with the given color.
/// If the user clicks the button, a full color picker is shown.
pub fn color_edit_button_rgb(ui: &mut Ui<'_>, rgb: &mut [f32; 3]) -> Response {
