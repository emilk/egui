...
impl Plot {
    ...
    fn show_dyn<'a, R>(
        ...
        if allow_drag && response.dragged_by(PointerButton::Primary) {
            response = response.on_hover_cursor(ui.ctx, CursorIcon::Grabbing);
        ...
        if allow_boxed_zoom {
            ...
            if let (Some(box_start_pos), Some(box_end_pos)) = (box_start_pos, box_end_pos) {
                ...
                if response.dragged_by(boxed_zoom_pointer) {
                    response = response.on_hover_cursor(ui.ctx, CursorIcon::ZoomIn);
        ...
        let response = if show_x || show_y {
            response.on_hover_cursor(ui.ctx, CursorIcon::Crosshair)
        } else {
            response
        };
    ...
    fn paint_axis(&self, ui: &Ui<'_>, axis: usize, shapes: &mut Vec<Shape>) {
        ...
        for step in steps {
            ...
            if text_alpha > 0.0 {
                ...
                if !text.is_empty() {
                    let galley = ui.ctx.fonts().layout_no_wrap(text, font_id.clone(), color);
