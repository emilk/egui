...
impl<'a> DragValue<'a> {
    ...
    /// Add decrement [-] and increment [+] buttons and set the step value.
    pub fn step_buttons<Num: emath::Numeric>(mut self, step: Num) -> Self {
        self.step = step.to_f64();
        self
    }
...
impl<'a> Widget for DragValue<'a> {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        let drag_id = ui.auto_id_with("drag");
        let is_slow_speed = ui.input().modifiers.shift_only();

        let old_value = get(&mut get_set_value);
        let show_precision =
            is_slow_speed && ui.memory().drag_value.last_dragged_id == Some(drag_id);
        ...
        let use_step_buttons = step > 0.;
        let auto_decimals = if use_step_buttons {
            ((aim_rad / speed.abs()).max(1. / step))
                .log10()
                .ceil()
        ...
        let round_decimals = auto_decimals + is_slow_speed as usize;
        let display_decimals = auto_decimals + show_precision as usize;

        let max_decimals = max_decimals.unwrap_or(display_decimals + 2);
        let display_decimals = display_decimals.clamp(min_decimals, max_decimals);
        let round_decimals = round_decimals.clamp(min_decimals, max_decimals);
        let value_text = if value == 0.0 {
            "0".to_owned()
        } else {
            emath::format_with_decimals_in_range(value, display_decimals..=max_decimals)
        };
        ...
        let mut response = ui
            .horizontal(|ui| {
                if is_kb_editing {
                    ...
                    let mut val_response = ui
                        .add(
                            Button::new(
                                RichText::new(format!("{}{}{}", prefix, value_text, suffix))
                                    .monospace(),
                            )
                            .id(drag_id)
                            .wrap(false)
                    ...
                    if ui.style().explanation_tooltips {
                        val_response = val_response.on_hover_text(format!(
                    ...
                    if val_response.clicked() {
                        ...
                        if delta_value != 0.0 {
                            ...
                            let stored_value = (drag_state.last_dragged_id
                                == Some(val_response.id))
                            ...
                            let rounded_new_value =
                                emath::round_to_decimals(rounded_new_value, round_decimals);
                            let rounded_new_value =
                                clamp_to_range(rounded_new_value, clamp_range.clone());
                            set(&mut get_set_value, rounded_new_value);

                            drag_state.last_dragged_id = Some(val_response.id);
                            drag_state.last_dragged_value = Some(stored_value);
                            ui.memory().drag_value = drag_state;
                        }
                    } else if val_response.has_focus() {
                        ...
                        if change != 0.0 {
                            ...
                            let new_value = emath::round_to_decimals(new_value, round_decimals);
                            let new_value = clamp_to_range(new_value, clamp_range.clone());
                ...
                if use_step_buttons {
                    ...
                    let step = emath::round_to_decimals(step, round_decimals);
                    let step = if step == 0.0 {
                        0.1_f64.powi(round_decimals.try_into().unwrap())
                    } else {
                        step
                    };

                    let minus_button_response =
                        ui.add_enabled(value > *clamp_range.start(), Button::new("➖"));
                    ...
                    let plus_button_response =
                        ui.add_enabled(value < *clamp_range.end(), Button::new("➕"));
